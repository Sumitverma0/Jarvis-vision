...
speak("Hello Sir, I am listening to you.")

while True:
    command = listen().lower()

    if "jarvis" in command:
        speak("How can I help you? Sir. ") # ok tested on 30-07-2015

    elif "time" in command:
        time = datetime.datetime.now().strftime("%I:%M %p")
        speak(f"Sir, the time is {time}")    # ok tested on 30-07-2015

    elif "open youtube" in command:
        speak("Sir,I Opening YouTube")
        webbrowser.open("https://www.youtube.com")   # ok tested on 30-07-2015
    
    elif "open notepad" in command:
        speak("Opening Notepad Sir")
        os.system("notepad.exe")     # ok tested on 30-07-2015

    elif "search" in command:
        speak("What should I search? Sir")
        search_query = listen().lower()
        webbrowser.open(f"https://www.google.com/search?q={search_query}")
        speak(f"Here are the results for {search_query}")     # ok tested on 30-07-2015
    ...
    #     start_hand_gesture_detection()
        ...
        speak(weather_info)  # ok tested on 30-07-2015


    elif "battery" in command or "battery status" in command:
        speak(get_battery_status())   # ok tested on 30-07-2015
        ...
        if user_query:
            ...
            speak("Sorry, I didn’t catch your question.")     # ok tested on 30-07-2015
